...
class EnergyMeterAdmin(admin.ModelAdmin):
    list_display = ('id', 'dp_count','id_ext', 'id_int_pp', 'id_int_old','factor', 'comment','in_operation_from','in_operation_to',)
    ...
    try:
        for attribute_key in AttributeKey.objects.filter(show_in_energymeter_admin=True):
            ...
            def get_attribute_key(instance, key_name=attribute_key.name):
                return instance.energymeterattribute_set.filter(key__name=key_name).first()
        ...
        for attribute_key in AttributeKey.objects.filter(show_from_mp_in_em_admin=True):
            ...
            def get_mp_attribute_key(instance, key_name=attribute_key.name):
                return instance.metering_point.meteringpointattribute_set.filter(key__name=key_name).first()
...
class MeteringPointAdmin(admin.ModelAdmin):
    list_display = ('id', 'name', 'is_sub_meter', 'energy_meters', 'utility','comment', )
    try:
        for attribute_key in AttributeKey.objects.filter(show_in_meteringpoint_admin=True):

            @admin.display(description=attribute_key.name)
            def get_attribute_key(instance, key_name=attribute_key.name):
                return instance.meteringpointattribute_set.filter(key__name=key_name).first()
    ...
    list_display_links = ('id',)
    list_editable = ('comment',)
    filter_horizontal = ('location','higher_level_metering_points', )
    list_filter = ['utility',]
...
class BuildingAdmin(admin.ModelAdmin):
    list_display = ('id', 'short_name','number', 'name', 'contruction_date', 'category', 'site',)# 'address__street', 'address__zip', 'address__town')
...
class AddressAdmin(admin.ModelAdmin):
    list_display = ('id', 'street', 'zip', 'town', )
    list_display_links = ('id',)
...
class BuildingInfoAdmin(admin.ModelAdmin):
    list_display = ('id', 'periode_from', 'periode_to', 'cost_unit','owner', 'area_net', 'area_HNF_1_6', 'area_NNF_7', 'area_FF_8', 'area_VF_9', 'nb_floors', 'nb_rooms')
